...
UserPage::UserPage(IUrlLauncher &urls, QWidget *parent) : QWidget(parent), urls_(urls) {
    ...
    description_=new QLabel("请到管理员页面创建或导入项目。",this);description_->setWordWrap(true);layout->addWidget(description_);
    ...
    logs_=new QPlainTextEdit(details);logs_->setReadOnly(true);logs_->setMaximumBlockCount(5000);logs_->setAccessibleName("最近运行日志");detailLayout->addWidget(logs_);
...
void UserPage::setState(ProjectState state) {
    ...
    main_->setStyleSheet("QPushButton { background-color: "+color+"; color: white; padding: 12px; } QPushButton:hover { background-color: "+QColor(color).lighter(108).name()+"; }");
