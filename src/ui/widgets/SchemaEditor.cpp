...
QString fieldHelp(const QString &key) {
    static const QMap<QString,QString> hints{
        ...
        {"workingDirectory","填写项目目录，不是 cmd.exe 或 Python 路径。留空继承源码目录；相对路径以源码目录为基准，路径无需外层引号。"},
        {"activationScript","仅 exec/shell 使用 .bat/.cmd，在同一进程中先激活再执行命令；相对路径以本命令的工作目录为基准。Python .venv 建议直接选择 python-venv 模式。"},
        {"venv","虚拟环境目录，例如 .venv（相对于命令工作目录）。使用其中的 Scripts/python.exe；缺失时先添加 python -m venv .venv 准备命令。"},
        {"mode","exec 原生程序；python-venv 环境解释器；uv 调用 uv run；npm 调用包脚本；shell 使用 cmd。"},
        {"program","只填写程序名或路径，不包含参数。npm 模式填写 node.exe（不是 npm.cmd）；uv 模式填写 uv.exe。参数在下方逐项添加。"},
